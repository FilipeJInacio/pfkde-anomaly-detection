...
handler = logging.FileHandler("godot/log.txt", mode='a')
...
for i, (current, nxt) in enumerate(zip(jsonData, jsonData[1:])):
    ...
    with open("godot/trajectory_bugsat.yml", "w") as out:
        yaml.dump(yamldata, out)

    uniConfig = cosmos.util.load_yaml('godot/universe_bugsat.yml')
    uni = cosmos.Universe(uniConfig)
    traConfig = cosmos.util.load_yaml('godot/trajectory_bugsat.yml')
